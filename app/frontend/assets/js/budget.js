...
(() => {
  ...
  (function applyAuthUi(){
    ...
    if (name){ st.textContent = `Привет, ${name}`; btnOut.style.display=''; } else { st.textContent='Гость'; btnOut.style.display='none'; }
  })();
  ...
  // CHANGED: используем /api в проде, а локально http://127.0.0.1:8000
  const baseURL =
  ...
  const fmtMoney = (v, cur='RUB') =>
    new Intl.NumberFormat('ru-RU', { style:'currency', currency:cur, maximumFractionDigits:2 }).format(Number(v||0));
  ...
  function fmtDate(val){
    ...
    const s = String(val); const p = s.split('-');
  ...
  const donutCenter = (getInfo) => ({
    ...
    beforeDraw(chart){
      ...
      const ctx = chart.ctx, doc = document.documentElement;
      const bg = getComputedStyle(doc).getPropertyValue('--card') || '#151935';
      const text = getComputedStyle(doc).getPropertyValue('--text') || '#ecf0ff';
      const muted = getComputedStyle(doc).getPropertyValue('--muted') || '#a0a7c9';

      ctx.save();
      ctx.beginPath(); ctx.arc(x,y,(innerR+outerR)/2*0.72,0,Math.PI*2); ctx.fillStyle=bg; ctx.fill();

      const info = getInfo(chart);
      ctx.textAlign='center'; ctx.textBaseline='middle';
      ctx.fillStyle = muted; ctx.font='700 12px Inter';
      if(info.title) ctx.fillText(info.title, x, y-18);
      ctx.fillStyle = text; ctx.font='800 16px Inter';
  ...
  // генератор HSL → градиент; разная палитра для доходов/расходов, и УНИКАЛЬНЫЙ цвет на категорию
  function hsl(h,s,l,a=1){ return `hsla(${h} ${s}% ${l}% / ${a})`; }
  function catPalette(ctx, labels, kind){
    const n = Math.max(labels.length, 1);
    const start = kind==='inc' ? 170 : 330;     // teal/синий vs розовый
    const spread = kind==='inc' ? 120 : 90;     // ширина палитры
    return labels.map((_,i)=>{
      const h = (start + (i*spread)/n) % 360;
      const g = ctx.createLinearGradient(0,0,0,260);
      g.addColorStop(0, hsl(h, 75, 60));
      g.addColorStop(1, hsl(h, 75, 42));
      return g;
    });
  }
  ...
  const commonTooltip = {
    callbacks:{
      label(c){
        ...
        const sum = (c.dataset.data||[]).reduce((s,x)=>s+Number(x||0),0) || 1;
        const p = v / sum * 100;
        return `${c.label}: ${fmtMoney(v)} (${nfPct.format(p)}%)`;
      }
    }
  };
  ...
  function upsertDoughnut(canvas, labels, values, total, kind){
    ...
    const colors = catPalette(ctx, labels, kind);      // <- уникальные цвета
    return new Chart(ctx, {
      type:'doughnut',
      data:{ labels, datasets:[{ data:values, borderWidth:0, hoverOffset:8, cutout:'62%', backgroundColor:colors }] },
      options:{ plugins:{ legend:{ display:false }, tooltip: commonTooltip }, animation:{ duration:900, easing:'easeOutQuart' } },
  ...
  function upsertExpenseLine(canvas, labels, values, year, month){
    ...
    const line = theme.getPropertyValue('--brand').trim() || '#6a7dff';
  ...
  async function loadSummary(signal){
    ...
    setText($('#kpiNet'), fmtMoney(data.net_total));       // «Остаток»
  ...
  async function loadCharts(signal){
    ...
    const mapByDay = {};
    for (const r of byDayRaw){ const dd=/^\d{4}-\d{2}-\d{2}/.test(r.name) ? parseInt(r.name.slice(8,10),10) : parseInt(r.name,10); if(!Number.isNaN(dd)) mapByDay[dd]=(mapByDay[dd]||0)+r.amount; }
  ...
  // ===== Obligations =====
  function isOverdue(dateISO, isDone){
  ...
  async function loadAccountsAndCats(signal){
    ...
    const accSel = $('#opAccount'); const contraSel = $('#opContra');
    if (accSel) accSel.innerHTML=''; if(contraSel) contraSel.innerHTML='';
    for (const a of ACCOUNTS){
      const title = `${a.title}${a.is_savings?' (накоп.)':''}`;
      accSel?.add(new Option(title, a.id)); contraSel?.add(new Option(title, a.id));
    }
    fillCategoriesForCurrentType();
  }
  ...
  $('#opTabs')?.addEventListener('click', (e)=>{
    ...
    document.querySelectorAll('#opTabs .tab-btn').forEach(b=>b.classList.remove('tab-btn--active'));
    t.classList.add('tab-btn--active');
    currentType = t.dataset.type;
    $('#opContraWrap')?.classList.toggle('hidden', currentType!=='transfer');
    $('#opCatRow')?.classList.toggle('hidden', currentType==='transfer');
    $('#opHint') && ($('#opHint').textContent = currentType==='transfer' ? 'Счета должны быть в одной валюте' : '');
    fillCategoriesForCurrentType();
  });
  function fillCategoriesForCurrentType(){
    const sel = $('#opCategory'); if(!sel) return; sel.innerHTML='';
    (currentType==='income' ? CATS_IN : CATS_EX).forEach(c=> sel.add(new Option(c.name, c.id)));
  }
  $('#opForm')?.addEventListener('submit', async (e)=>{
    e.preventDefault();
    const payload = {
      type: currentType,
      occurred_at: $('#opDate').value || new Date().toISOString().slice(0,10),
      account_id: Number($('#opAccount').value),
      amount: Number($('#opAmount').value || '0'),
      description: $('#opDesc').value || null
    };
    if (currentType === 'transfer') payload.contra_account_id = Number($('#opContra').value);
    else payload.category_id = Number($('#opCategory').value);
    try{
      await api.post('/budget/transactions', payload);
      $('#opAmount').value=''; $('#opDesc').value='';
      await refreshAll('op-submit');
  ...
  $('#catTabs')?.addEventListener('click', (e)=>{
    ...
    t.classList.add('tab-btn--active');
    currentKind = t.dataset.kind; renderCatTable();
  });
  $('#catForm')?.addEventListener('submit', async (e)=>{
    e.preventDefault();
    const name = $('#catName').value.trim(); if(!name) return;
    try{
      await api.post('/budget/categories', { kind: currentKind, name });
      $('#catName').value = '';
  ...
  // ===== Obligations actions =====
  $('#addObBtn')?.addEventListener('click', async ()=>{
    const title = $('#obTitle').value.trim();
    const due_date = $('#obDate').value;
    ...
    $('#obTitle').value=''; $('#obDate').value=''; $('#obAmount').value='';
    await loadObligations(activeAbort?.signal);
  });
  $('#obTable')?.addEventListener('change', async (e)=>{
    const t = e.target;
    if (t instanceof HTMLInputElement && t.classList.contains('ob-done')){
      const id = t.dataset.id;
      await api.patch(`/budget/obligations/${id}`, { is_done: t.checked });
      await loadObligations(activeAbort?.signal); // чтобы цвета/классы обновились
    }
  });
  $('#obTable')?.addEventListener('click', async (e)=>{
    const t = e.target; if(!(t instanceof HTMLElement)) return;
    if (t.dataset.act === 'del'){
      const id = t.dataset.id; await api.delete(`/budget/obligations/${id}`); await loadObligations(activeAbort?.signal);
    }
  });
  ...
  toggleAllBtn?.addEventListener('click', ()=>{
    const open = anyOpen();
    document.querySelectorAll('details.collapsible').forEach(d=> d.open = !open);
    updateToggleAllBtn();
  });
  updateToggleAllBtn();
  ...
  (async ()=>{
    ...
    if (opDate) opDate.valueAsNumber = Date.now() - (new Date()).getTimezoneOffset()*60000;
