...
(() => {
  ...
  const applyCompact = () =>
    headerEl?.classList.toggle('header--compact', window.scrollY > 8);
  ...
  // локально — 127.0.0.1:8000, в проде — /api (nginx → backend)
  ...
  async function ping(signal){
    try{ await api.get('/health/ping',{signal}); setApiIndicator(true); }
  ...
  const donutCenter = (getInfo) => ({
    ...
    beforeDraw(chart){
      ...
      ctx.save();
      ctx.beginPath(); ctx.arc(x, y, Math.max(innerR-2,0), 0, Math.PI*2);
  ...
  const commonTooltip = {
    callbacks:{ label(c){
      ...
      const p=v/sum*100; return `${c.label}: ${fmtMoney(v)} (${nfPct.format(p)}%)`;
    } }
  };
  ...
  function upsertExpenseLine(canvas, labels, values, year, month){
    ...
    g.addColorStop(0,'rgba(67,97,238,.35)'); g.addColorStop(1,'rgba(67,97,238,.05)');
    ...
    return new Chart(ctx,{
      ...
      options:{
        plugins:{ legend:{ display:false }, tooltip:{
          ...
          callbacks:{
            title(items){ const d=Number(items[0].label||'0');
              const mm=String(month).padStart(2,'0'); const dd=String(d).padStart(2,'0');
              return `${dd}.${mm}.${year}`; },
        ...
        scales:{
          ...
          y:{ ticks:{ callback:(v)=> new Intl.NumberFormat('ru-RU').format(v) },
              grid:{ color: theme.getPropertyValue('--stroke') || 'rgba(0,0,0,.1)' } }
        },
  ...
  async function loadSummary(signal){
    ...
    const { data } = await api.get('/budget/summary/month',
      { params:{ date_from:from, date_to:to }, signal });
  ...
  async function loadCharts(signal){
    ...
    const { data } = await api.get('/budget/summary/charts',
      { params:{ date_from:from, date_to:to }, signal });
  ...
  function isOverdue(dateISO, isDone){
    if (isDone || !dateISO) return false;
  ...
  function fillAccountSelects(){
    ...
    if (ACCOUNTS.length){
      ...
      acc.value = first; contra.value = first;
    }
  }
  ...
  async function loadMonthTransactions(signal){
    ...
    const res = await api.get('/budget/transactions',
      { params:{ date_from:from, date_to:to, limit:500 }, signal });
  ...
  function renderLedger(){
    ...
    if (tab==='income'){
      $('#ledgerIncome')?.classList.add('visible');
      $('#ledgerExpense')?.classList.remove('visible');
      setText($('#tabTotal'), fmtMoney(s1));
    } else {
      $('#ledgerExpense')?.classList.add('visible');
      $('#ledgerIncome')?.classList.remove('visible');
      setText($('#tabTotal'), fmtMoney(s2));
    }
  }
  ...
  $('#ledgerTabs')?.addEventListener('click', (e)=>{
    const t = e.target;
    if(!(t instanceof HTMLElement) || !t.classList.contains('tab-btn')) return;
  ...
  $('#catTable')?.addEventListener('click', async (e)=>{
    ...
    if (t.dataset.act === 'del-cat'){
      ...
      try{
        ...
        alert(err?.response?.data?.detail || 'Нельзя удалить категорию (есть операции?)');
      }
    }
  });
  ...
  function openModal(sel, e){
    ...
    m.removeAttribute('hidden');            // критично для WebView
  ...
  document.querySelectorAll('.modal [data-close]')?.forEach(btn=>{
    btn.addEventListener('click', (ev)=>{
      const root = ev.currentTarget.closest('.modal');
      if(root){ root.setAttribute('hidden',''); document.body.style.overflow=''; }
    });
  });
  window.addEventListener('keydown', (ev)=>{
    if (ev.key === 'Escape'){
      document.querySelectorAll('.modal:not([hidden])').forEach(m => m.setAttribute('hidden',''));
      document.body.style.overflow = '';
    }
  });
  ...
  $('#opmTabs')?.addEventListener('click', (e)=>{
    const t=e.target; if(!(t instanceof HTMLElement) || !t.classList.contains('seg-btn')) return;
    setModalType(t.dataset.type);
  });
  function fillCategoriesForModal(){
    const sel = $('#m_cat'); if(!sel) return;
    sel.innerHTML='';
    const list = modalType==='income' ? CATS_IN : CATS_EX;
  ...
  $('#opmForm')?.addEventListener('submit', async (e)=>{
    ...
    try{
      ...
      closeModal('#opModal'); await refreshAll('op-submit');
    }catch(err){
      alert(err?.response?.data?.detail || 'Ошибка при добавлении операции');
    }
  });

  // ====== МОДАЛКА: Категория ======
  function setModalKind(k){
    modalKind = k;
    document.querySelectorAll('#catmTabs .seg-btn').forEach(b=> b.classList.toggle('is-active', b.dataset.kind===k));
  }
  $('#catmTabs')?.addEventListener('click', (e)=>{
    const t=e.target; if(!(t instanceof HTMLElement) || !t.classList.contains('seg-btn')) return;
    setModalKind(t.dataset.kind);
  });
  $('#catmForm')?.addEventListener('submit', async (e)=>{
    ...
    try{
      ...
      alert(err?.response?.data?.detail || 'Ошибка при добавлении категории');
    }
  });
  ...
  async function addObligation(){
    ...
    try{
      ...
      if ($('#obTitle')) $('#obTitle').value=''; if ($('#obDate')) $('#obDate').value='';
      if ($('#obAmount')) $('#obAmount').value='';
      await loadObligations(activeAbort?.signal);
    }catch(err){
      alert(err?.response?.data?.detail || 'Не удалось добавить платёж');
    }
  }
  ...
  document.getElementById('obTable')?.addEventListener('click', async (e)=>{
    const btn = (e.target instanceof HTMLElement) ? e.target.closest('[data-act="del"]') : null;
    if (!btn) return;
    const id = btn.getAttribute('data-id'); if (!id) return;
    if (!confirm('Удалить платёж?')) return;
    try{
      await api.delete(`/budget/obligations/${id}`); await loadObligations(activeAbort?.signal);
    }catch(err){
      alert(err?.response?.data?.detail || 'Не удалось удалить платёж');
    }
  });
  document.getElementById('obTable')?.addEventListener('change', async (e)=>{
    const chk = e.target;
    if (!(chk instanceof HTMLInputElement) || !chk.classList.contains('ob-done')) return;
    const id = chk.getAttribute('data-id'); const date = chk.getAttribute('data-date') || null;
    try{
      await api.patch(`/budget/obligations/${id}`, { is_done: chk.checked, date });
      await loadObligations(activeAbort?.signal);
    }catch(err){
  ...
  async function refreshAll(_reason='manual'){
    ...
    try{
      ...
      isRefreshing=false; scheduleNext();
    }
  }
